...
- `Editor/LFShapeEditor.cs` (assembly `libfive.Editor`): custom inspector, pickable/outline gizmos (`[DrawGizmo]`), undo hook, `Tools ▸ libfive` and `GameObject ▸ libfive` menus.

Assembly definitions: `Assets/libfive/libfive.asmdef` (runtime, unsafe code, references Burst/Collections/Mathematics), `Scripts/Editor/libfive.Editor.asmdef`, `Tests/Editor/libfive.Tests.asmdef`.
